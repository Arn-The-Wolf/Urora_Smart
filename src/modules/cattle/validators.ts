...
const photoUrlsField = z.array(z.string().trim().min(1).max(700_000)).max(6).optional();

export const cowInputSchema = z
  .object({
  ...
  .superRefine((value, ctx) => {
    const photos = value.photoUrls?.length ? value.photoUrls : value.photoUrl ? [value.photoUrl] : [];
    if (!photos.length) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["photoUrls"],
        message: "Every cow needs at least one photo",
      });
    }
  });
